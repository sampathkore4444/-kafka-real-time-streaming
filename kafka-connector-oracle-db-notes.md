...
- C##DEBEZIUM (COMMON = YES) — this is a **common user** (exists in CDB and all PDBs).
- DEBEZIUM (COMMON = NO) — this is a **local user** (only in XEPDB1).
...
Now it will work perfectly with the common user — this is what was missing in the previous steps.

Your CDC pipeline is complete! 🚀

Insert data in `BANK.TRANSACTIONS` and watch Kafka topics fill up. Let me know when you see the events! 🎉
